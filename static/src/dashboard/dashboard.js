...
class AwesomeDashboard extends Component {
	...
	setup(){
		...
		this.statistics = useState(this.tshirtService.statistics);

		this.keyToString = {
			average_quantity: this.env._t("Averge amount of t-shirt by order this month"),
			average_time: this.env._t(
				"Average time for an order to go from 'new' to 'sent' or 'cancelled'"
			),
			nb_cancelled_order: this.env._t("Number of cancelled orders this month"),
	...
	openLast7DaysOrders(){
		const domain = 
		"[('create-date', '>=', (context_today() - datetime.timedelta(days=7)).strftime('%Y-%m-%d'))]";
		this.openOrders(this.env._t("Last 7 days orders", domain));
	}

	openLast7DaysCancelledOrders(){
		const domain = 
		"[('create-date', '>=', (context_today() - datetime.timedelta(days=7)).strftime('%Y-%m-%d')), ('state', '=', 'cancelled')]";
