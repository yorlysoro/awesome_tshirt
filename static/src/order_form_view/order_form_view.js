...
class OrderFormController extends FormController {
	setup(){
		...
		this.debouncedPrintLabel = useService(this.printLabel, 200);
	}

	async printLabel(){
		conts serverResult = await this.orm.call(this.model.root.resModel, "print_label", [
	...
	get isPrintBtnPrimary(){
		return (
				...
				this.mode.root.data.state === "printed"
			);
	}
}
